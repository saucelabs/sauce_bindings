...
public abstract class VDCConfigurations<T extends VDCConfigurations<T>>
    extends BaseConfigurations<T> {
  ...
  /**
   * Toggle on requirement that input elements with type=file need to be displayed and enabled in
   * order to use Keep the default if the input fields on your site are never displayed Set with
   * this method if you have an automated synchronization strategy that the default incorrectly
   * bypasses
   *
   * @return instance of configuration
   */
  ...
  /**
   * @param behavior enum for desired response when sending non-alert commands with an alert
   *     displayed
   * @return instance of configuration
   */
  ...
  /**
   * Toggles off Sauce Labs recording video of the session. Not recommended since videos are very
   * useful for debugging failures and visual reproduction of actions taken. If performance is the
   * only concern, there is a (small) time savings during the test by disabling this
   *
   * @return instance of configuration
   */
  ...
  /**
   * Toggles off Sauce Labs video post-processing and uploading when test is reported passing If
   * performance is the only concern, there is a (small) time savings after the test by disabling
   * this
   *
   * @return instance of configuration
   */
  ...
  /**
   * Toggles off the step-by-step screenshots Sauce Labs takes during a test run If performance is
   * the only concern, there is a (small) time savings during the test by disabling this
   *
   * @return instance of configuration
   */
  ...
  /**
   * Toggles off Sauce Labs recording logs of user commands If performance is the only concern,
   * there is a (small) time savings during the test by disabling this Note that the Selenium Logs
   * will still be recorded and available
   *
   * @return instance of configuration
   */
  ...
  /**
   * This is a safety measure to prevent tests from running indefinitely The default is 30 minutes;
   * the maximum is 180 minutes
   *
  ...
  /**
   * As a safety measure to prevent tests from running too long after something has gone wrong The
   * default is 90 seconds; the maximum is 1000 seconds
   *
  ...
  /**
   * For faster performance, you may want to upload the executable to Sauce Storage, a private
   * temporary storage space
   *
   * @param prerun a Map of the configurations for downloading and executing a file before the test
   *     begins
   * @return instance of configuration
  ...
  /**
   * If you have multiple new jobs waiting to start, jobs with a lower priority number take
   * precedence over jobs with a higher number Sauce will attempt to find resources for Jobs with
   * priority 0 before priority 1, etc This can be useful if you have a test that runs as part of a
   * build and you don't want it to wait for a daily run to complete
   *
